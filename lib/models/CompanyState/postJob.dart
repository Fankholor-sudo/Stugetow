...
import 'package:stugetow/models/CompanyState/postJobSettings.dart';
...
class WorkPost extends StatefulWidget {
  WorkPost({Key? key}) : super(key: key);
...
class _WorkPostState extends State<WorkPost> {
  ...
  bool? _settingsDisabledApplications;
  ...
  static var now = new TimeOfDay.fromDateTime(
      DateTime.parse(_postDate.toString()));
  ...
  Widget build(BuildContext context) {
    return _inProcess? Loading():Scaffold(
      appBar: new AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.check, color: (!_isTyping && !_isImageSelected) ? Colors.grey[500] : Colors.blueAccent[200]),
            iconSize: 30.0,
            onPressed: () async {
              if(_isTyping || _isImageSelected) {
                setState((){_inProcess=true;});
                dynamic result = await _auth.postCompany(
                    userParam: _comp,
                    date: dateFormat.format(_postDate),
                    time: _postTime,
                    ...
                    allowApplications: _settingsDisabledApplications
                );
                if(result!=false) {
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              Navigator.push(
                                  ...
                                  builder: (builder) =>
                                      PostJobSettings(notifyParent: refresh))
                                // AnimatedPageRouteSlide(page: PostJobSettings()),
                              );
                            },
                          ),
                        ],
  ...
  void initState() {
    ...
    setState(() {
      ...
      _settingsDisabledApplications = false;
    });
  }
  ...
  refresh(dynamic commentDisabled, dynamic applicationDisabled) {
    setState(() {
      _settingsDisabledComment = commentDisabled;
      _settingsDisabledApplications = applicationDisabled;
      print('comment: ' + _settingsDisabledComment.toString() +
          '\napplications: ' + _settingsDisabledApplications.toString());
    });
  }
  ...
  _getUserDetails() async {
    ...
    setState((){
      ...
      String? imageUrl = result['profileImg'];
      String? email = result['email'];
      String? role = result['role'];
      _comp = new Comp(name, email, description, role, imageUrl);
    });
  }
